...
function formatTickLabel(datetime, spanMs) {
  if (!datetime) {
    return '--';
  }

  const label = String(datetime);
...
function buildTicks(points, width, padding) {
  if (!points.length) {
    return [];
  }
  ...
  if (points.length === 1) {
    return [
      {
        x: points[0].x,
        label: formatTickLabel(points[0].datetime, 0),
      },
    ];
  }
  ...
  const ticks = [first, middle, last]
    .map((point) => ({
      ...
      label: formatTickLabel(point.datetime, spanMs),
    }))
...
export function buildChartModel({
  ...
  const mappedPoints = normalized.map((point, index) => {
    const x = normalized.length === 1
      ? width / 2
      : padding + (((point.timestamp - minTimestamp) / timestampSpan) * (width - (padding * 2)));
  ...
  return {
    ...
    points: mappedPoints,
    ticks: buildTicks(mappedPoints, width, padding),
    summary,
  };
}
